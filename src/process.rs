    ...
    T: SizedSample + FromSample<f64>,
{
    ...
    let c = 0.2 * (dsp::organ_hz(dsp::midi_hz(57.0)) + dsp::organ_hz(dsp::midi_hz(61.0)) + dsp::organ_hz(dsp::midi_hz(64.0)));
    let c = c >> dsp::pan(0.0);
    let c = c >> (dsp::chorus(0, 0.0, 0.01, 0.2) | dsp::chorus(1, 0.0, 0.01, 0.2));
    let mut c = c
        >> (dsp::declick() | dsp::declick())
        >> (dsp::dcblock() | dsp::dcblock())
        >> dsp::limiter_stereo((1.0, 5.0));
